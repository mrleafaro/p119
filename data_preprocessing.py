...
def get_stem_words(words, ignore_words):
    stem_words = []
    for word in words:
        # Verifique se a palavra não faz parte da lista de palavras a serem ignoradas
        if word not in ignore_words:
            # Converta a palavra para minúsculo
            word = word.lower()
            # Stemize a palavra usando o objeto stemmer
            stemmed_word = stemmer.stem(word)
            # Anexe a palavra stemizada à lista stem_words
...
def preprocess_train_data():
    ...
    with open('stem_words.pickle', 'wb') as file:
        pickle.dump(stem_words, file)

    with open('tag_classes.pickle', 'wb') as file:
